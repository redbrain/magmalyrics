...
module.exports = class {
    ...
    async getLyrics (search, source=this.options.default) {
        ...
        try {
            let connection = await fetch(conf.search.url+encodeURIComponent(search),{headers:{'User-Agent':this.options.useragent},timeout:this.options.timeout});
            let doc = await connection.text();
            let $ = cheerio.load(doc);
            const urlElement = $(conf.search.select).first();
            const url = urlElement.prop('href');
            connection = await fetch(new URL(url,conf.search.url),{headers:{'User-Agent':this.options.useragent},timeout:this.options.timeout});
